...
button.addEventListener("click", async () => {
	...
	topbar.style.top = "0";
	container.innerHTML = "<p class=\"inter italic text-2xl text-gray-100\">loading...</p>";
	mapelement.style.display = "block";
	maptilersdk.config.apiKey = "MD47r9kN2owa1tUJVi75"; // I'm not dumb, no. This *is* an API key, but it's a useless one. It just indicates what account to grab the style from. In this case, it's my account, and it's grabbing my red map style.
	...
	try {
				...
				boxesinrange.forEach(store => {
					...
					if (!markers.has(id)) {
						...
						if (storecoords) {
							...
							if (!district) {
								district = "(unknown hub/district)";
							}
							if (!currentstatus) {
								currentstatus = "(unknown)";
							}
							if (!notes) {
								notes = "(no note availiable)";
							}
							...
							const popup = new maptilersdk.Popup({
								offset: 25
							}).setHTML(`<h1 class="text-2xl inter font-extrabold text-black">${name}</h1><h2 class="text-md inter text-gray-700">${address}</h2><p class="text-sm inter text-black">status: ${currentstatus}</p><p class="text-sm inter text-black">open date: ${opendate}</p><p class="text-sm inter text-black">hub/district: ${district}</p><p class="text-sm inter text-black">notes:</p><textarea disabled>${notes}</textarea>`)
		...
		container.innerHTML = "<p class=\"inter italic text-2xl text-gray-100\">an error occurred during the last map load.</p>";
	}
});